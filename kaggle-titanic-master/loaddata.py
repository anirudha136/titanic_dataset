...
def setMissingAges():
    ...
    age_df = df[['Age','Embarked','Fare', 'Parch', 'SibSp','Pclass','Names','CabinLetter']]
    ...
    for i in range(0, numerics.columns.size-1):
        for j in range(0, numerics.columns.size-1):
            if i <= j:
                ...
                df = pd.concat([df, pd.DataFrame(pd.Series(numerics.iloc[:,i] * numerics.iloc[:,j], name=name))], axis=1)
                new_fields_count += 1
            if i < j:
                name = str(numerics.columns.values[i]) + "+" + str(numerics.columns.values[j])
                df = pd.concat([df, pd.DataFrame(pd.Series(numerics.iloc[:,i] + numerics.iloc[:,j], name=name))], axis=1)
...
def reduceAndCluster(input_df, submit_df, clusters=3):
    ...
    df = pd.concat([pd.DataFrame(survivedSeries), pd.DataFrame(clusterIdSeries), pcaDataFrame], axis=1)
...
if __name__ == '__main__':
    ...
    df_results_train = pd.DataFrame(train)
    df_results_test = pd.DataFrame(test)
    df_results_train.to_csv('titanic_train',index=False)
    df_results_test.to_csv('titanic_test',index=False)
    exit()
